...
import { TunnelCluster, TunnelClusterOption } from "./TunnelCluster";
...
export interface TunnelOption extends TunnelClusterOption {
  host: string;
  port: string;
  subdomain: string;
}
...
export class Tunnel extends EventEmitter {
  ...
  url: string;
  cachedUrl: string;
  constructor(opts: TunnelOption) {
    super(opts);
    this.opts = opts;
    this.closed = false;
    if (!this.opts.host) {
      this.opts.host = "https://tunnel.builderhub.io";
    }
  }
  ...
  _init(cb: (err: Error | null, data?: any) => void) {
    const opt = this.opts;
    const getInfo = this._getInfo.bind(this);

    const params: { responseType: ResponseType } = {
      responseType: "json",
    };
  ...
  open(cb: (err?: Error | null, data?: any) => void) {
    this._init((err, info) => {
      if (err) {
        return cb(err);
      }
